...
const UploadDataset = () => {
  ...
  const loadLatestDataset = async () => {
    try {
      ...
      console.error("Failed to load latest dataset:", err.message);
    }
  };
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      setDatasetMeta({
        ...
        numericColumns: res.data.numericColumns || [],
        rowCount: res.data.rowCount || 0,
        targetColumns: res.data.targetColumns || detectedColumns.slice(1, 6),
      });

      setShowParameters(true); // 🔥 THIS WAS MISSING
  ...
  const handleSaveDataset = async () => {
    try {
      ...
      const saveResponse = await API.post("/dataset/save", {
        fileName,
        filePath,
        datasetType: datasetMeta?.datasetType,
        columns,
        numericColumns: datasetMeta?.numericColumns,
        rowCount: datasetMeta?.rowCount,
        targetColumns: datasetMeta?.targetColumns || columns.slice(1, 6),
      });
      localStorage.setItem("lastDatasetId", saveResponse.data.dataset._id);
      ...
      if (!account) {
        toast.warning(
          "Wallet not connected. Dataset saved locally but not on blockchain.",
        );
        return;
      }
